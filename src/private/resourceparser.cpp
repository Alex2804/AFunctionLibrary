                                          ...
                                          std::vector<std::shared_ptr<TokenWrapper<std::string>>> &tokens)
{
    ...
    for(const pugi::xml_node& child : node.children()) {
        ...
        if(childName == "extension") {
            ...
            defaultBundle.aliases = std::vector<TokenAliases>();
            parseTokensRecursive(child, std::move(defaultBundle), tokens);
        }
    }
}
...
void afl::detail::parseTokenAliases(std::string aliasString, TokenBundle<std::string> &bundle,
                                    afl::detail::TokenAliasType type)
{
    ...
    if(iterator == bundle.aliases.end()) {
        bundle.aliases.push_back(TokenAliases{type, std::set<std::string>()});
                                       ...
                                       std::vector<std::shared_ptr<TokenWrapper<std::string>>> &tokens)
{
    ...
    for(const pugi::xml_attribute& attribute : node.attributes()) {
        if(strcmp(attribute.name(), "value") != 0 && strcmp(attribute.name(), "values") != 0) {
            ...
            parseTokenField(pugi::xml_node(), attribute.name(), attribute, token, tokens);
        } else {
    ...
    for(const pugi::xml_node& child : node.children()) {
        if(strcmp(child.name(), "token") == 0) {
            parseTokensRecursive(child, token, tokens);
        } else { // parse node
            if(child.type() == pugi::xml_node_type::node_pcdata)
                childName = "values";
            else
                childName = child.name();
            parseTokenField(child, childName, child.text(), token, tokens);
        }
    }

    for(const pugi::xml_attribute& attribute : attributes) {
        std::string name = attribute.name();
        parseTokenField(pugi::xml_node(), attribute.name(), attribute, token, tokens); // value is at end of list ... attributes are assigned first
    }
}
